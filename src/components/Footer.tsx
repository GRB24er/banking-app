      ...
      <div className={styles.footerTop}>
        <div className={styles.container}>
          <div className={styles.brandSection}>
            {/* Logo Section - Properly Separated */}
          ...
          <div className={styles.linksGrid}>
            <div className={styles.linkSection}>
              ...
              <ul className={styles.linkList}>
                {["Checking Accounts", "Savings Accounts", "Credit Cards", "Personal Loans", "Mortgages", "Auto Loans"].map((item, idx) => (
            ...
            <div className={styles.linkSection}>
              ...
              <ul className={styles.linkList}>
                {["Business Accounts", "Commercial Loans", "Merchant Services", "Payroll Solutions", "Treasury Management", "Business Credit"].map((item, idx) => (
                  <motion.li key={idx} whileHover={{ x: 5 }}>
            ...
            <div className={styles.linkSection}>
              ...
              <ul className={styles.linkList}>
                {["Investment Services", "Retirement Planning", "Trust Services", "Private Banking", "Estate Planning", "Tax Advisory"].map((item, idx) => (
                  <motion.li key={idx} whileHover={{ x: 5 }}>
                    <Link href={`/wealth/${item.toLowerCase().replace(/\s+/g, "-")}`}>
                      <span className={styles.linkBullet}>•</span>
                      {item}
                    </Link>
                  </motion.li>
            ...
            <div className={styles.linkSection}>
              ...
              <ul className={styles.linkList}>
                {["Help Center", "Security Center", "Rates & Fees", "Find Locations", "Contact Us", "Careers"].map((item, idx) => (
                  <motion.li key={idx} whileHover={{ x: 5 }}>
                    <Link href={`/${item.toLowerCase().replace(/\s+/g, "-")}`}>
                      <span className={styles.linkBullet}>•</span>
                      {item}
          ...
          <div className={styles.contactSection}>
            ...
            {subscribeStatus === "success" && (
              <motion.p className={styles.successMessage} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                ✓ You're subscribed to financial updates.
              </motion.p>
            )}

            {subscribeStatus === "error" && (
              <motion.p className={styles.errorMessage} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                We couldn't process your request. Please try again.
              </motion.p>
            )}

            <div className={styles.contactInfo}>
              <motion.div className={styles.contactItem} whileHover={{ backgroundColor: "rgba(212, 175, 55, 0.08)" }}>
        ...
        <div className={styles.container}>
          <div className={styles.badgesGrid}>
            {[
              ...
              <motion.div key={idx} className={styles.badge} whileHover={{ y: -5 }}>
          ...
          <div className={styles.bottomContent}>
            <div className={styles.copyright}>
              <p className={styles.copyrightText}>
                © {currentYear} Horizon Global Capital Bank plc. All rights reserved.
              </p>
            ...
            <div className={styles.legalLinks}>
              {["Privacy Policy", "Terms of Use", "Security Center", "Regulatory Disclosures", "Cookie Preferences", "Sitemap"].map((link, idx) => (
                <motion.div key={idx} whileHover={{ scale: 1.05 }}>
                  <Link href={`/${link.toLowerCase().replace(/\s+/g, "-")}`} className={styles.legalLink}>
                    {link}
                  </Link>
